    ...
    public static void saveUsers(List<User> users) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(USERS_FILE))) {
            for (User u : users) {
                ...
                pw.println(type + "," + u.getId() + "," + u.getName()
    ...
    public static void saveProducts(List<Product> products) {
        try (PrintWriter pw = new PrintWriter(new FileWriter(PRODUCTS_FILE))) {
            for (Product pr : products) {
                String cat = pr.getCategory() != null ? pr.getCategory() : "General";
                if (pr instanceof AuctionProduct) {
                    AuctionProduct ap = (AuctionProduct) pr;
                    pw.println("AUCTION," + ap.getId() + "," + ap.getName() + ","
                            + ap.getStartingPrice() + "," + ap.getCurrentBid() + ","
                            + ap.getSellerId() + "," + ap.getEndTime() + "," + cat);
                } else {
                    pw.println("PRODUCT," + pr.getId() + "," + pr.getName() + ","
                            + pr.getPrice() + "," + pr.getStock() + ","
                            + pr.getSellerId() + "," + cat);
                }
            }
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(ORDERS_FILE))) {
            for (Order o : orders) {
                ...
                for (CartItem ci : o.getItems())
                    sb.append(",").append(ci.getProduct().getId()).append(":").append(ci.getQuantity());
                pw.println(sb);
            }
